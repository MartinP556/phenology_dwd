...
def phase_dependent_response2(driver_values, t_dev, responses, thresholds):
    ...
    #t_dev is the (cts) development time
    response = np.zeros(driver_values.shape)
    response += (t_dev <= thresholds[0])*responses[0](driver_values)
    for phase_index in range(len(responses) - 1):
        response += (t_dev >= thresholds[phase_index])*(t_dev <= thresholds[phase_index + 1])*responses[phase_index](driver_values) #First brackets indicates if we are at the right phase, second takes the response function for each phase
    return response
